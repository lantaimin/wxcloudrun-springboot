...
public class MemoryUserRecordSpace {
    ...
    private final ConcurrentHashMap<String, RecordsArray> userRecordMap = new ConcurrentHashMap<>();

    public ConcurrentHashMap<String, RecordsArray> getUserRecordMap() {
        return userRecordMap;
    }

    public List<MsgDTO> getAllInteractMsg(String id) {
    ...
    public void storeInteractMsg(String id, InteractMsg interactMsg) {
        RecordsArray recordsArray = userRecordMap.get(id);
        // 为空说明不存在该用户的记录
        if (recordsArray == null) {
    ...
    private RecordsArray storeUserRecord(String id, InteractMsg interactMsg) {
        // 判断是否满了
    ...
    private RecordsArray storeUserRecord(String id){
